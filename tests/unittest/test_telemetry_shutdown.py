...
from pr_agent.telemetry import registry as registry_module
...
def test_flush_providers_share_one_deadline(monkeypatch):
    """Later providers receive only the budget left after earlier flushes."""
    clock = iter([1_000_000_000, 1_400_000_000])
    monkeypatch.setattr(registry_module.time, "monotonic_ns", lambda: next(clock))
    tracer_provider = _DeadlineRecordingProvider()
    meter_provider = _DeadlineRecordingProvider()
    provider_registry.register(tracer_provider)
    provider_registry.register(meter_provider)

    flush_telemetry(timeout_millis=1000)

    assert tracer_provider.timeout_millis == 1000
    assert meter_provider.timeout_millis == 600


def test_flush_attempts_later_provider_with_zero_when_deadline_is_exhausted(monkeypatch):
    """An exhausted shared deadline must not restore the full timeout or skip a provider."""
    ...
    assert tracer_provider.timeout_millis == 1000
    assert meter_provider.timeout_millis == 0
