...
from pr_agent.telemetry.config import VALID_EXPORTER_TYPES, _parse_otlp_headers, get_otel_config, parse_otel_bool
...
@pytest.mark.parametrize("value", [False, None, "", "false", "FALSE", "0", "no", "off", "unexpected"])
def test_telemetry_boolean_parser_fails_closed(value):
    assert parse_otel_bool(value) is False


@pytest.mark.parametrize("value", [True, "true", "TRUE", "1", "yes", "on"])
def test_telemetry_boolean_parser_accepts_explicit_opt_in(value):
    assert parse_otel_bool(value) is True


def test_false_string_disables_telemetry(monkeypatch):
    _use_settings(monkeypatch, {**VALID_ENABLED_SETTINGS, "OTEL.IS_ENABLED": "false"})

    assert get_otel_config().is_enabled is False
