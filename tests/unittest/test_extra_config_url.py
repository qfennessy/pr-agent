...
def test_extra_config_load_is_request_local_and_not_process_cached(tmp_path, settings_sandbox, monkeypatch):
    extra_path = _write_toml(tmp_path, "extra.toml", f"[{_TEST_SECTION}]\nvalue = 'shared'\n")
    get_settings().set("CONFIG.EXTRA_CONFIG_URL", extra_path)
    applied = []
    monkeypatch.setattr(
        git_utils,
        "_apply_settings_from_file",
        lambda path, label: applied.append((path, label)),
    )
    monkeypatch.setattr(git_utils, "context", {})

    git_utils.apply_extra_config_settings()
    git_utils.apply_extra_config_settings()

    assert applied == [(extra_path, "extra")]

    # A new request context must load the same source again; no process-global
    # cache may carry settings freshness assumptions across requests.
    monkeypatch.setattr(git_utils, "context", {})
    git_utils.apply_extra_config_settings()

    assert applied == [(extra_path, "extra"), (extra_path, "extra")]
