        ...
        bindings.append(
            ProductionArmBinding(
                ...
                telemetry_shape=(
                    ...
                    else ModelTelemetryShape.SINGLE_SELECTED
                ),
                adapter=adapter,
                available=True,
            )
...
def test_binding_rejects_wrong_model_identity_and_per_role_telemetry(tmp_path):
    snapshot, path, artifact_hash = _write_snapshot(tmp_path)
    manifest = _manifest(snapshot, artifact_hash)
    request, decision = _paid_authorization(manifest)
    store = _NoCallStore()
    ...
    bindings[specialist_index] = replace(
        ...
        telemetry_shape=ModelTelemetryShape.PER_STAGE,
    )
