...
class TestPushOutputs:
    ...
    def test_channel_failures_do_not_prevent_later_sinks(self, monkeypatch, tmp_path):
        get_settings().set('PUSH_OUTPUTS.ENABLE', True)
        ...
        monkeypatch.setattr(utils, 'print', fail_stdout, raising=False)
        monkeypatch.setattr(utils, 'open', fail_file, raising=False)
        monkeypatch.setattr(utils.requests, 'post', post)

        push_outputs("review", payload={"a": 1}, markdown="still delivered")

        assert posts == [('https://example.test/slack', {"text": "still delivered"})]

    def test_http_error_does_not_prevent_later_sink(self, monkeypatch):
        get_settings().set('PUSH_OUTPUTS.ENABLE', True)
        ...
        def post(url, **kwargs):
            response = MagicMock()
            if url.endswith('/webhook'):
                response.raise_for_status.side_effect = utils.requests.HTTPError("503")
            else:
        ...
        warnings = []
        monkeypatch.setattr(utils.requests, 'post', post)
        monkeypatch.setattr(utils.get_logger(), 'warning', warnings.append)

        push_outputs("review", payload={"a": 1}, markdown="delivered after 503")

        assert delivered == [('https://example.test/slack', {"text": "delivered after 503"})]
        assert warnings == ["push_outputs webhook failed: HTTPError"]
    ...
    def test_non_https_sink_urls_are_ignored(self, monkeypatch, bad_url):
        ...
        monkeypatch.setattr(utils.requests, 'post',
                            lambda url, **kwargs: posts.append(url) or MagicMock())
