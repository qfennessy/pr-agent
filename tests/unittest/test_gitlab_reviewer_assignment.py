...
class TestGetBotUserId:
    ...
    def test_caches_by_credential(self):
        ...
        with mock.patch("pr_agent.servers.gitlab_webhook.global_settings",
                        self._make_settings("https://a.example.com", "token-a")):
            with mock.patch.dict("sys.modules", {"gitlab": self._make_fake_gitlab(111)}):
                assert asyncio.run(_get_bot_user_id()) == 111

        with mock.patch("pr_agent.servers.gitlab_webhook.global_settings",
                        self._make_settings("https://a.example.com", "token-b")):
    ...
    def test_no_cache_on_failure(self):
        ...
        with mock.patch("pr_agent.servers.gitlab_webhook.global_settings",
                        self._make_settings("https://x.example.com", "fail-token")):
    ...
    def test_respects_auth_type_private_token(self):
        ...
        with mock.patch("pr_agent.servers.gitlab_webhook.global_settings", s):
    ...
    def test_ignores_request_local_gitlab_auth_overrides(self):
        from pr_agent.servers.gitlab_webhook import _bot_user_id_cache, _get_bot_user_id
        _bot_user_id_cache.clear()

        host_settings = self._make_settings("https://gitlab.host.example", "host-token")
        request_settings = self._make_settings("https://attacker.example", "request-token")
        fake_gitlab = self._make_fake_gitlab(42)

        with (
            mock.patch("pr_agent.servers.gitlab_webhook.global_settings", host_settings),
            mock.patch("pr_agent.servers.gitlab_webhook.get_settings", return_value=request_settings),
            mock.patch.dict("sys.modules", {"gitlab": fake_gitlab}),
        ):
            assert asyncio.run(_get_bot_user_id()) == 42

        assert fake_gitlab.Gitlab.call_args.kwargs == {
            "url": "https://gitlab.host.example",
            "oauth_token": "host-token",
            "ssl_verify": True,
        }

    def test_preserves_extra_config_auth_before_repo_overrides(self):
        from pr_agent.servers import gitlab_webhook

        gitlab_webhook._bot_user_id_cache.clear()
        values = {}
        request_settings = mock.MagicMock()
        request_settings.get.side_effect = lambda key, default=None: values.get(key, default)
        request_context = {}
        fake_gitlab = self._make_fake_gitlab(43)

        def apply_extra_config():
            values.update({
                "GITLAB.URL": "https://gitlab.extra.example",
                "GITLAB.PERSONAL_ACCESS_TOKEN": "host-token",
                "GITLAB.SSL_VERIFY": True,
                "GITLAB.AUTH_TYPE": "private_token",
            })

        def apply_repo_settings(url, *, include_extra_config=True):
            assert url == "https://gitlab.extra.example/group/project/-/merge_requests/1"
            assert include_extra_config is False
            values.update({
                "GITLAB.URL": "https://attacker.example",
                "GITLAB.PERSONAL_ACCESS_TOKEN": "attacker-token",
            })

        with (
            mock.patch.object(gitlab_webhook, "context", request_context),
            mock.patch.object(gitlab_webhook, "get_settings", return_value=request_settings),
            mock.patch.object(gitlab_webhook, "apply_extra_config_settings", side_effect=apply_extra_config),
            mock.patch.object(gitlab_webhook, "apply_repo_settings", side_effect=apply_repo_settings),
            mock.patch.dict("sys.modules", {"gitlab": fake_gitlab}),
        ):
            gitlab_webhook._apply_repo_settings_with_host_auth(
                "https://gitlab.extra.example/group/project/-/merge_requests/1"
            )
            assert asyncio.run(gitlab_webhook._get_bot_user_id()) == 43

        assert fake_gitlab.Gitlab.call_args.kwargs == {
            "url": "https://gitlab.extra.example",
            "private_token": "host-token",
            "ssl_verify": True,
        }
