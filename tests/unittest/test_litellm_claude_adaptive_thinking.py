...
def _settings(reasoning_effort="medium", enabled=False, extended_enabled=False, config_overrides=None):
    flags = {
        "enable_claude_adaptive_thinking": enabled,
        "enable_claude_extended_thinking": extended_enabled,
    }
    flags.update(config_overrides or {})
...
async def _run_completion(monkeypatch, model, reasoning_effort="medium", enabled=False,
                          extended_enabled=False, extended_override=None, config_overrides=None):
    monkeypatch.setattr(
        litellm_handler,
        "get_settings",
        lambda: _settings(reasoning_effort, enabled, extended_enabled, config_overrides),
    )
    with patch(
...
def test_detects_adaptive_thinking_models_across_providers(model, expected):
    assert litellm_handler.LiteLLMAIHandler._is_claude_adaptive_thinking_model(model) is expected
...
@pytest.mark.asyncio
async def test_adaptive_thinking_normalizes_xhigh_to_supported_max(monkeypatch, model):
    kwargs = await _run_completion(
        monkeypatch,
        model,
...
@pytest.mark.asyncio
async def test_adaptive_thinking_string_false_is_disabled(monkeypatch):
    kwargs = await _run_completion(
        monkeypatch,
        "anthropic/claude-opus-4-8",
        reasoning_effort="high",
...
@pytest.mark.asyncio
async def test_adaptive_model_bypasses_invalid_legacy_budget_and_keeps_generic_cap(monkeypatch):
    kwargs = await _run_completion(
        monkeypatch,
        "anthropic/claude-opus-5",
        reasoning_effort="high",
