...
def restore_github_settings():
    ...
    had_artifacts = "ARTIFACTS" in settings
    original_artifacts = copy.deepcopy(settings.get("ARTIFACTS", None))
    ...
    if had_artifacts:
        settings.set("ARTIFACTS", original_artifacts)
    else:
...
@pytest.mark.asyncio
async def test_workflow_run_ci_conclusion_ignores_artifact_target_tools(
    monkeypatch, tmp_path, restore_github_settings
):
    runs = []
    _patch_workflow_run_deps(monkeypatch, runs)
    get_settings().set("ARTIFACTS.TARGET_TOOLS", ["pr_description"])
    monkeypatch.setenv("GITHUB_EVENT_NAME", "workflow_run")
    monkeypatch.setenv(
        "GITHUB_EVENT_PATH", str(_write_workflow_run_event(tmp_path, conclusion="failure"))
    )
    ...
    for tool in ("pr_reviewer", "pr_description", "pr_code_suggestions"):
        assert "concluded: failure" in str(get_settings().get(tool).extra_instructions)
