...
def test_pinecone_comment_ids_remain_one_based_and_decode_to_list_indexes():
    assert psi._pinecone_comment_record_id("issue_42", 0) == "issue_42.comment_1"
    assert psi._pinecone_comment_record_id("issue_42", 2) == "issue_42.comment_3"
    assert psi._pinecone_comment_index("issue_42.comment_1") == 0
    assert psi._pinecone_comment_index("issue_42.comment_3") == 2


@pytest.mark.parametrize("record_id", ["issue_42.comment_0", "issue_42.comment_bad", "issue_42"])
def test_pinecone_comment_ids_fail_closed_when_they_are_not_historical_ids(record_id):
    assert psi._pinecone_comment_index(record_id) is None
...
@pytest.mark.asyncio
async def test_pinecone_stale_comment_id_falls_back_to_issue_url(monkeypatch):
    posted = []
    ...
    class FakeIndex:
        def query(self, *args, **kwargs):
            return SimpleNamespace(
                to_dict=lambda: {
                    "matches": [{"id": "issue_2.comment_2", "score": 0.9}],
                }
            )
    ...
    issue_main = SimpleNamespace(
        title="Current issue",
        body="Body",
        number=1,
        get_comments=lambda: [],
        create_comment=posted.append,
    )
    similar_issue = SimpleNamespace(
        title="Similar issue",
        html_url="https://example.test/issues/2",
        get_comments=lambda: [SimpleNamespace(html_url="https://example.test/issues/2#comment-1")],
    )
    tool = psi.PRSimilarIssue.__new__(psi.PRSimilarIssue)
    tool.supported = True
    tool.issue_url = "https://github.com/example/repo/issues/1"
    ...
    tool.git_provider = SimpleNamespace(
        _parse_issue_url=lambda url: ("example/repo", 1),
        repo_obj=SimpleNamespace(get_issue=lambda number: issue_main if number == 1 else similar_issue),
    )

    assert await tool.run() is None
    assert len(posted) == 1
    assert "https://example.test/issues/2" in posted[0]
    assert "#comment-1" not in posted[0]
