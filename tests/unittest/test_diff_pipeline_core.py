...
class TestPrGenerateCompressedDiff:
    ...
    def test_duplicate_filename_patches_are_combined_before_pruning(self, monkeypatch):
        monkeypatch.setattr(pr_processing, "get_max_tokens", lambda model: 10_000)
        staged = _make_file(
            filename="tracked.py",
            patch="@@ -1,1 +1,1 @@\n-old\n+staged-value\n",
        ...
        (
            ...
            top_langs=[{"files": [staged, unstaged]}],
            token_handler=FakeTokenHandler(prompt_tokens=10),
            model="some-model",
            convert_hunks_to_line_numbers=False,
            large_pr_handling=False,
        ...
        assert "worktree-value" in file_dict["tracked.py"]["patch"]
