...
from tests.unittest._telemetry_helpers import build_in_memory_tracer, clear_telemetry_caches

SENTINEL_URL = "https://secret-repo.example/org/private-repo/pull/42"

_TRACKED_KEYS = ("otel.include_error_details", "otel.include_pr_url")
...
@pytest.mark.asyncio
async def test_host_extra_config_enables_telemetry_before_tracer_cache(tmp_path, monkeypatch):
    tracked_keys = (
    ...
    extra_config = tmp_path / "host-extra.toml"
    extra_config.write_text(
        """
[otel]
is_enabled = true
exporter_type = "none"
service_name = "extra-config-test"
environment = "test"
"""
    )
    settings.set("CONFIG.EXTRA_CONFIG_URL", str(extra_config))
    settings.set("OTEL.IS_ENABLED", False)
    clear_telemetry_caches()
    events = []
    real_apply_extra_config = pr_agent_module.apply_extra_config_settings
    real_get_tracer = tracer_module.get_tracer

    def apply_extra_config():
        real_apply_extra_config()
        events.append(("extra", settings.get("OTEL.IS_ENABLED")))

    def get_tracer():
        tracer = real_get_tracer()
        events.append(("tracer", settings.get("OTEL.IS_ENABLED"), tracer))
        return tracer

    def apply_repo_settings(_pr_url, *, include_extra_config=True):
        events.append(("repo", include_extra_config))

    monkeypatch.setattr(pr_agent_module, "apply_extra_config_settings", apply_extra_config)
    monkeypatch.setattr(pr_agent_module, "get_tracer", get_tracer)
    monkeypatch.setattr(pr_agent_module, "apply_repo_settings", apply_repo_settings)

    try:
        handled = await pr_agent_module.PRAgent()._handle_request(
            "https://example/pr/1", ["unknown-command"]
        )

        assert handled is False
        assert [event[0] for event in events] == ["extra", "tracer", "repo"]
        assert events[0] == ("extra", True)
        assert events[1][1] is True
        assert not isinstance(events[1][2], NoOpTracer)
        assert events[2] == ("repo", False)
        assert real_get_tracer.cache_info().currsize == 1
    finally:
        clear_telemetry_caches()
        restore_settings(snapshot)
...
async def test_unknown_command_text_not_exported_by_default(telemetry):
    ...
    handled = await pr_agent_module.PRAgent()._handle_request("https://example/pr/1", [SENTINEL_URL])
...
async def test_unknown_command_text_exported_when_opted_in(telemetry):
    ...
    await pr_agent_module.PRAgent()._handle_request("https://example/pr/1", [SENTINEL_URL])
...
@pytest.mark.asyncio
async def test_false_string_does_not_export_pr_url_or_error_details(telemetry):
    exporter, _ = telemetry
    get_settings().set("otel.include_error_details", "false")
    get_settings().set("otel.include_pr_url", "false")

    await pr_agent_module.PRAgent()._handle_request(SENTINEL_URL, [SENTINEL_URL])

    attrs = dict(exporter.get_finished_spans()[0].attributes)
    assert "pr_agent.pr_url" not in attrs
...
async def test_handle_request_flushes_telemetry_even_on_failure(telemetry, monkeypatch):
    ...
    monkeypatch.setattr(pr_agent_module.PRAgent, "_handle_request", exploding)
    await pr_agent_module.PRAgent().handle_request("https://example/pr/1", "/review")
    assert flushes == [True], "failed requests must still flush telemetry"

    async def ok(self, pr_url, request, notify=None):
        return True

    monkeypatch.setattr(pr_agent_module.PRAgent, "_handle_request", ok)
    await pr_agent_module.PRAgent().handle_request("https://example/pr/1", "/review")
...
async def test_tool_exception_not_auto_recorded_by_default(telemetry, monkeypatch):
    ...
    handled = await pr_agent_module.PRAgent(ai_handler="fake").handle_request(
        "https://example/pr/1", ["customcmd"]
    )
...
async def test_propagating_exception_recorded_when_opted_in(telemetry, monkeypatch):
    ...
    await pr_agent_module.PRAgent(ai_handler="fake").handle_request("https://example/pr/1", ["customcmd"])
